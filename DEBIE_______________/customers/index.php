...
<div class="page">
    ...
    <a href="./?page=add_Customer" class="btn-download">
        ...
        <tbody>
            ...
            if ($result->num_rows > 0) {
                ...
                while($row = $result->fetch_assoc()) {
                    ...
                    echo "<td><div class='image-container'><img src='".validate_image($row["image"])."' alt='Customer Image'></div></td>";
                    
                    // Display status with appropriate styling
                    if ($row["status"] == "non_active") {
                        echo "<td class='not-available'>Blocked</td>";
                        echo "<td><a href='delete.php?table=customer&status=unblock&id=".$row["id"]."' onclick='return confirm(\"You are going to unblock this user?\")'>Unblock</a> | <a href='./?page=add_Customer&id=".$row["id"]."'>Update</a> | <a href='delete.php?table=customer&id=".$row["id"]."' onclick='return confirm(\"Are you sure you want to delete this record?\")'>Delete</a></td>"; // Action buttons
                    } else {
                        echo "<td class='available'>Active</td>";
                        echo "<td><a href='delete.php?table=customer&status=block&id=".$row["id"]."' onclick='return confirm(\"You are going to block this user?\")'>Block</a> | <a href='./?page=add_Customer&id=".$row["id"]."'>Update</a> | <a href='delete.php?table=customer&id=".$row["id"]."' onclick='return confirm(\"Are you sure you want to delete this record?\")'>Delete</a></td>"; // Action buttons
                    }
                    echo "</tr>";
                }
            } else {
